...
import * as bindings from "serverless-azure-functions/lib/shared/bindings.json";
...
if (!bindings.bindings.some(({ type }) => type === "eventGrid")) {
    ...
    actions: [
      {
        ...
        settings: ["topicEndpointUri", "topicKeySetting"],
      },
    ],
    documentation: "$content=Documentation\\eventGridTrigger.md",
    extension: {
  ...
  bindings.bindings.push(eventGridOutBinding as never);
}
